...
alter policy "lesson assets owner select" on storage.objects
  using (bucket_id = 'lesson-assets' and (storage.foldername(name))[1] = (select auth.uid())::text and (select auth.uid()) = '225f2092-e96f-4065-bf8f-0d68d7c3cf78'::uuid);
alter policy "lesson assets owner insert" on storage.objects
  with check (bucket_id = 'lesson-assets' and (storage.foldername(name))[1] = (select auth.uid())::text and (select auth.uid()) = '225f2092-e96f-4065-bf8f-0d68d7c3cf78'::uuid);
